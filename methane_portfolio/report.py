...
def generate_appendix(
    ...
    *,
    ...
    actual_lam = lam if lam is not None else 0.5
    actual_alpha = alpha if alpha is not None else 0.90
    actual_delta = delta if delta is not None else 0.10

    manifest_path = out / "run_manifest.json"
    if manifest_path.exists() and any(
        v is None for v in [chains, draws, tune, target_accept, lam, alpha, delta]
    ):
        try:
            ...
            if lam is None and "lam" in params:
                actual_lam = params["lam"]
            if alpha is None and "alpha" in params:
                actual_alpha = params["alpha"]
            if delta is None and "delta" in params:
                actual_delta = params["delta"]
        except Exception:
