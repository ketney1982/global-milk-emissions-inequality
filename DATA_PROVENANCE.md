...
| Release | `v3.0.1-R3` (3 September 2026) — the version that produces **and regenerates** the reported results. A file inside a commit cannot carry that commit's own hash, so this record identifies the release by tag; resolve it with `git rev-list -n 1 v3.0.1-R3`, which dereferences the annotated tag to its commit (plain `git rev-parse` returns the tag object, not the commit). `v3.0.1-R3` differs from `v3.0.0-R3` only in the deposited verification manifest, which now records a clean working tree at commit `0059918d0c64fa32fc1cf0f886537e85ea1fdbea` — the commit that produced the deposited outputs — instead of the pre-release tree it was first written from, and in this document; no code, no input and no reported value differs between the two tags. The preceding releases are tag `v2.0.0-R2` (2 September 2026), which produced the same numbers but whose public entry point did not regenerate the full sensitivity grid or export the reported absolute-reduction quantity, and commit `57d1c4cd3b5110df6f1c511fb338d50bb6d50f90` (message "R1", 18 February 2026), which accompanied the preprint. `CHANGELOG.md` lists what changed at each step. `v2.0.0-R2`, `v3.0.0-R3` and `v3.0.1-R3` are all annotated Git tags in the repository, not GitHub Release objects. |
...
7. Country-level posterior quantities depend on the country effects through `u_c = tau * u_c_raw`, which meets the strict convergence criteria at every one of the 182 country levels. The likelihood constrains the product `tau * u_c_raw` far more tightly than either factor, so the two are weakly identified from the likelihood and both mix slowly; the zero-sum prior fixes the scale of the raw offsets, so `tau` is formally identified and the diagnostics indicate weak identification and poor posterior geometry rather than structural non-identifiability. Consequently the **magnitude of between-country dispersion** is not well determined and is not interpreted as a substantive result anywhere in this study.
